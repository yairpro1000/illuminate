...
describe('book review coupon UI', () => {
  ...
  it('renders coupon editing controls on the paid booking review step', () => {
    ...
    expect(html).toContain('Pay now via Stripe')
  ...
  it('skips payment-choice copy when the computed final price is zero', () => {
    const views = window.BookPageViews.createBookPageViews({
      ctx: {
        source: '1_on_1',
        mode: 'new',
      ...
      isIntroFlow: () => false,
      isSessionPayNowFlow: () => true,
      isZeroPriceFlow: () => true,
      slotWindowMonths: 4,
    })

    const html = views.buildShell().replace(/\s+/g, ' ')
    expect(html).toContain('Free booking — email confirmation required')
