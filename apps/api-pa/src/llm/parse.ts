...
async function openAiParse(
  ...
  transcript: string,
  opts: { apiKey: string; model: string; requestId?: string; logger?: ObservabilityLogger },
...
async function openAiRefineUpdateDelete(
  ...
  }, opts.logger, "refine_update_delete");
...
export async function parseTranscript(
  schema: SchemaRegistry,
  transcript: string,
  opts: { provider: Provider; openaiApiKey?: string; openaiModel?: string; requestId?: string; candidates?: Array<{ id: string; text: string }>; logger?: ObservabilityLogger },
): Promise<ParsedAction> {
  ...
  switch (opts.provider) {
    case "openai": {
      ...
      let action = await openAiParse(schema, transcript, { apiKey, model, requestId: opts.requestId, logger: opts.logger });
      if ((action.type === "update_item" || action.type === "delete_item") && opts.candidates) {
        action = await openAiRefineUpdateDelete(schema, transcript, action, opts.candidates, { apiKey, model, requestId: opts.requestId, logger: opts.logger });
      }
      return action;
    }
    case "heuristic":
...
export async function refineUpdateDelete(
  ...
  draft: ParsedAction,
  candidates: Array<{ id: string; text: string }>,
  opts: { openaiApiKey: string; openaiModel?: string; requestId?: string; logger?: ObservabilityLogger },
) {
  const model = opts.openaiModel ?? "gpt-4.1-mini";
