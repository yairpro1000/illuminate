...
import { attachObservability, getLogger, getRequestId, persistFrontendLog, makeRequestId } from "./observability";
...
type PublicSourceType = "booking" | "registration";
type BookingType = "intro" | "session";
type BookingSource = "session" | "event";
...
function ensurePublicType(raw: string | undefined | null): PublicSourceType {
  return raw === "registration" ? "registration" : "booking";
}

function ensureBookingType(raw: string | undefined | null): BookingType {
  return raw === "session" ? "session" : "intro";
}

function ensureBookingSource(raw: string | undefined | null): BookingSource {
  return raw === "event" ? "event" : "session";
}

function getClientOrigin(request: Request) {
  const originHeader = request.headers.get("origin");
  if (originHeader && /^https?:\/\//i.test(originHeader)) return originHeader.replace(/\/$/, "");
  const u = new URL(request.url);
  return `${u.protocol}//${u.host}`;
}

function getOffsetForTimezone(day: string, tz: string) {
  // Use noon UTC for stable DST resolution on a given date.
  ...
  const m = offset.match(/GMT([+-]\d{1,2})(?::?(\d{2}))?/i);
  if (!m) return "+00:00";
  const hh = m[1].startsWith("+") || m[1].startsWith("-") ? m[1] : `+${m[1]}`;
  const sign = hh.startsWith("-") ? "-" : "+";
  const absHour = hh.replace(/[+-]/g, "").padStart(2, "0");
  const mm = (m[2] ?? "00").padStart(2, "0");
  return `${sign}${absHour}:${mm}`;
}

function isoForLocalSlot(day: string, hour: number, minute: number, tz: string) {
  const offset = getOffsetForTimezone(day, tz);
...
async function verifyTurnstile(
  ...
  if (token === "placeholder") return true;
...
function canReschedule(status: string) {
  return status === "pending_payment" || status === "confirmed" || status === "cash_ok";
}
...
function splitFullName(name: string) {
  ...
  if (!trimmed) return { firstName: "Unknown", lastName: null as string | null };
...
async function upsertClient(
  ...
  const existing = Array.isArray(existingRows) && existingRows.length ? (existingRows[0] as ClientRecord) : null;
  if (existing) {
    const updatePayload = {
      ...payload,
  ...
  return inserted as ClientRecord;
}
...
function isLocalhostRequestUrl(rawUrl: string): boolean {
  try {
...
function asHttpError(status: number, message: string): Error {
  const err = new Error(message);
  (err as any).status = status;
  return err;
}

function requireAdminAccess(c: any): { email: string } {
  const { email } = requireAccess(c);
  const allowlist = parseAdminAllowlist(c.env.ADMIN_ALLOWED_EMAILS);
  if (allowlist.size > 0) {
    if (!allowlist.has(normalizeEmail(email))) {
      throw asHttpError(403, "Admin access denied.");
    }
    return { email };
  }

  const devEmail = c.env.PA_DEV_EMAIL;
  if (
    typeof devEmail === "string" &&
    devEmail.trim() &&
    isLocalhostRequestUrl(c.req.url) &&
    normalizeEmail(devEmail) === normalizeEmail(email)
  ) {
    return { email };
  }
  ...
  throw asHttpError(
    403,
    "Admin access is not configured. Set ADMIN_ALLOWED_EMAILS or use localhost with PA_DEV_EMAIL.",
  );
}

function toOrganizerBookingRow(raw: any): OrganizerApiBookingRow {
  const clientRaw = Array.isArray(raw.clients) ? raw.clients[0] : raw.clients;
  const eventRaw = Array.isArray(raw.events) ? raw.events[0] : raw.events;
  return {
    booking_id: String(raw.id),
    source: String(raw.source) === "event" ? "event" : "session",
    status: String(raw.status ?? ""),
    event_id: raw.event_id ? String(raw.event_id) : null,
    event_title: eventRaw?.title ? String(eventRaw.title) : null,
    starts_at: String(raw.starts_at),
    ends_at: String(raw.ends_at),
    timezone: String(raw.timezone ?? "Europe/Zurich"),
    attended: Boolean(raw.attended),
    notes: typeof raw.notes === "string" ? raw.notes : null,
    client_id: clientRaw?.id ? String(clientRaw.id) : String(raw.client_id),
    client_first_name: String(clientRaw?.first_name ?? ""),
    client_last_name: typeof clientRaw?.last_name === "string" ? clientRaw.last_name : null,
    client_email: String(clientRaw?.email ?? ""),
    client_phone: typeof clientRaw?.phone === "string" ? clientRaw.phone : null,
  };
}
