...
* `entity` object: `{ booking_id, registration_id, payment_id, event_id }` as available
...
* In DB tables (bookings/registrations), you store full email—that’s business data.
...
* `route`, `method`
* `operation`
* `booking_id`, `event_id`, `registration_id`, `payment_id`
...
export function log_event({
  ...
  const payload = {
    ...
    route,
    entity,            // { booking_id, registration_id, payment_id }
    duration_ms,
    details,
    error
  };
...
export async function log_failure_to_db({
  ...
  await db.from("failure_logs").insert({
    ...
    event_id: entity.event_id || null,
    event_registration_id: entity.registration_id || null,
