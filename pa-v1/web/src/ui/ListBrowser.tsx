...
export function ListBrowser(props: { refreshSignal: number }) {
  ...
  async function loadLists() {
    setErr(null);
    const data = await api<{ lists: ListInfo[] }>("/api/lists", { method: "GET" });
    setLists(data.lists);
    if (!listId) {
  ...
  async function loadItems(id: string) {
    ...
    setErr(null);
    try {
      const data = await api<{ items: ListItem[] }>(`/api/lists/${encodeURIComponent(id)}/items`, {
        method: "GET",
      });
      setItems(data.items);
    } catch (e: any) {
  ...
  async function loadAllRows() {
    ...
    try {
      const pairs = await Promise.all(
        lists.map(async (l) => {
          const data = await api<{ items: ListItem[] }>(`/api/lists/${encodeURIComponent(l.id)}/items`, {
            method: "GET",
          });
  ...
  async function commit(action: any, opts?: { refresh?: boolean }) {
    await api("/api/commit", { method: "POST", body: JSON.stringify({ action }) });
  ...
  async function saveReorder(orderedIds: string[]) {
    await api(`/api/lists/${encodeURIComponent(listId)}/reorder`, {
      method: "POST",
        ...
        <div className="row" style={{ marginBottom: 10 }}>
          ...
          <div className="col">
            <div className="btnrow" style={{ justifyContent: "flex-end" }}>
              <a href={`/api/export/${encodeURIComponent(listId)}.csv`} className="pill small">
                Download CSV
              </a>
              <a href={`/api/export/${encodeURIComponent(listId)}.xlsx`} className="pill small">
                Download XLSX
              </a>
            </div>
          </div>
