---
name: illuminate-ui
description: Apply the Illuminate design system (color tokens, typography, layout, components, light/dark mode) to a new page, app, or website. Covers both the full user-facing UI and the lean admin/backoffice variant.
argument-hint: [page, file, or feature to style]
allowed-tools: Read, Glob, Grep, Edit, Write, Bash
---

You are applying the Illuminate design system to $ARGUMENTS. Your job is to produce clean, production-quality HTML/CSS that feels like it belongs in the same world as the Illuminate site — same soul, adapted to the new context. Follow every convention below precisely.
...
**Then stop and confirm with the user.** Write a single short message in this exact format — nothing else, no code:
...
Use this for external, user-facing pages: landing pages, booking flows, marketing pages, public features.
...
1. **Dark by default.** Deep navy backgrounds, white-ish text. Light mode is an opt-in override.
2. **Glassmorphism on cards.** Always use `backdrop-filter: blur(12px)` + semi-transparent bg on cards, modals, and nav when scrolled.
3. **Teal is the only accent color.** Use it for CTAs, focus rings, active states, borders on hover.
...
- [ ] Light mode override block in `body.theme-light {}`
...
Use this for backoffice and internal pages. Same color tokens, typography, light/dark mode, and responsiveness as the full system — but strips out everything that exists only for visual delight. Admin pages are tools, not experiences.

**Keep:** color system, typography, layout, BEM naming, light/dark mode toggle, responsive breakpoints, form input patterns, focus rings.

**Remove:** all `transition`, `animation`, `@keyframes`; `backdrop-filter`/glassmorphism; `box-shadow` glows; `transform` hover effects; gradient backgrounds; `clamp()` display sizes; scroll reveal (`.fade-up`); `--ease`/`--ease-spring` variables.

---

### Admin Tokens

Add alongside the standard color variables:
...
.btn-primary:hover { background: var(--color-lake); }
...
.btn-danger:hover { background: oklch(50% 0.20 25); }
~~~
...
.admin-table tr:hover td { background: var(--color-lake-mist); }
~~~
...
1. **No transitions or animations.** State changes are instant. Zero `transition:`, `animation:`, or `@keyframes`.
...
4. **No glow shadows.** Remove `--shadow-glow`. Use `--shadow-card` only for dropdowns if truly needed.
5. **No hover transforms.** No `translateY`, `scale`, or `perspective`. Hover only changes `background` or `border-color`.
6. **Compact sizing.** Reduce padding, use `0.875rem` body text and `0.75rem` for table/label text.
...
10. **Same responsive approach.** Use the same `48rem` breakpoint. On mobile the sidebar collapses to an off-canvas drawer.
...
| File | Contents |
|------|----------|
| `apps/site/css/main.css` | Full design system — source of truth |
...
When in doubt, read `apps/site/css/main.css` before writing anything.
