...
Goal: replace the temporary file-based PA storage that existed earlier with a **lean relational schema** in Supabase Postgres, while keeping PA V1 behavior.

The old `data/pa/*` JSONL bootstrap files have been removed from the repo. The database-backed PA model below is the only storage direction that should be treated as current.
