...
3. Create side effects: `create_stripe_checkout`, `reserve_slot`, `expire_booking`.
4. On settlement (`PAYMENT_SETTLED`), status advances to `PAID` and slot is confirmed.

### 1:1 Pay-Later

1. Create booking (`PENDING_CONFIRMATION`).
2. Emit `BOOKING_FORM_SUBMITTED_PAY_LATER`.
3. Create side effects: `send_email_confirmation`, `reserve_slot`, `expire_booking`.
4. On `EMAIL_CONFIRMED`, slot is confirmed.
5. Payment reminder scheduling follows policy (`starts_at - 24h` threshold).
...
3. Create side effects: `send_email_confirmation`, `reserve_slot`, `expire_booking`.
4. On `EMAIL_CONFIRMED`, status advances to `SLOT_CONFIRMED`.
